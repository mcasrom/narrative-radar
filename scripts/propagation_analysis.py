...
if os.path.exists(HISTORY):
    ...
    hist = hist[(pd.to_datetime(hist["date"], errors="coerce", utc=True) >= (pd.Timestamp.now() - pd.Timedelta(days=90))) & (pd.to_datetime(hist["date"], errors="coerce", utc=True) <= pd.Timestamp.now())]
