...
for script in \
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "[$timestamp]  $script finalizó con error o timeout" >> "$LOG_FILE"
    fi
done
...
for script in purge_history.py personas_tracking.py mass_media_analysis.py detect_narratives.py detect_coordination.py agenda_setting.py build_network.py propagation_analysis.py; do
    echo "[$timestamp] → Ejecutando $script ..." >> "$LOG_FILE"
...
if git diff --cached --quiet; then
    echo "[$timestamp]  No hay cambios nuevos para subir" >> "$LOG_FILE"
else
